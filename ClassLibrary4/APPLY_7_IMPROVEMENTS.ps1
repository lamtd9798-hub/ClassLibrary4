    ...
    [int]$ExpectedMin = 1)
{
    ...
    $start = 0
    while ($true) {
        $i = $text.IndexOf($Old, $start, [System.StringComparison]::Ordinal)
        if ($i -lt 0) { break }
        $count++
        $start = $i + $Old.Length
    }
    ...
    [string]$Label)
{
    ...
    $newText = $text.Substring(0, $i) + $New + $text.Substring($i + $Old.Length)
    ...
    [string]$Label)
{
    ...
    $newText = $text.Substring(0, $i + $Anchor.Length) + $Insert + $text.Substring($i + $Anchor.Length)
    Write-Utf8 $Path $newText
    Write-Host "[OK] $Label"
}
